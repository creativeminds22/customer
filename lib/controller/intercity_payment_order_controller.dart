...
import 'package:customer/payment/paypal/PayPalCurrencyCodeErrorModel.dart' as payPalCurrModel;
import 'package:customer/payment/paypal/paypalErrorSettle.dart';
import 'package:customer/payment/paypal/paypalPaymentSettle.dart' as payPalSettel;
import 'package:customer/payment/paypal/paypalclientToken.dart';
...
class IntercityPaymentOrderController extends GetxController {
  ...
  completeOrder() async {
    ...
    await FireStoreUtils.setWalletTransaction(adminCommissionWallet).then((value) async {
      if (value == true) {
        await FireStoreUtils.updateDriverWallet(
            amount: "-${Constant.calculateAdminCommission(amount: (double.parse(orderModel.value.finalRate.toString()) - double.parse(couponAmount.toString())).toString())}",
    ...
    await SendNotification.sendOneNotification(
        token: driverUserModel.value.fcmToken.toString(), title: 'Payment completed', body: '${userModel.value.fullName} has completed your payment', payload: {});
  ...
  paypalPayment(String amount) async {
    PayPalClientTokenGen.paypalClientToken(paypalSettingData: paymentModel.value.paypal!).then((value) async {
      final String tokenizationKey = paymentModel.value.paypal!.braintreeTokenizationKey.toString();
      log(tokenizationKey);
      var request = BraintreePayPalRequest(amount: amount, currencyCode: "USD", billingAgreementDescription: "India", displayName: 'GoRide company');
      ...
      if (resultData?.nonce != null) {
        PayPalClientTokenGen.paypalSettleAmount(
          paypalSettingData: paymentModel.value.paypal!,
          ...
          if (value['success'] == "true" || value['success'] == true) {
            if (value['data']['success'] == "true" || value['data']['success'] == true) {
              ...
              ShowToastDialog.showToast(settleResult.data.message);
            }
          } else {
        ...
        ShowToastDialog.showToast("Payment Incomplete!!");
      }
    });
  }
